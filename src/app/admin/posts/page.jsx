...
export default function Page() {
  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        const api = await postServices.getByPageSize({
          "pagination[page]": page,
          "pagination[pageSize]": PAGES_SIZE,
        });
        setPosts(api?.data || api || []);
        setPagination(api?.pagination || api?.meta || null);
      } catch (err) {
        console.error(err);
        setError("Lỗi không tìm thấy bài viết");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleDel(rowid) {
    ...
    try {
      await postServices.delete(rowid);
      alert("Xóa bài viết thành công!");
      setPosts((prev) => prev.filter((item) => item.id !== rowid));
    } catch (error) {
      console.error(error);
      alert("Lỗi xóa bài viết trên Backend API!");
    }
  }
  ...
  if (loading) return <div className="flex h-full items-center justify-center"><h1 className="text-center text-2xl font-bold">Loading Danh Sách Bài Viết...</h1></div>;
  ...
  return (
    <>
      <nav className="mx-6 mt-6 mb-2 flex items-center space-x-2 text-sm font-medium text-gray-500">
        ...
        <span>/</span>
        <span className="text-gray-800">Quản lý bài viết</span>
      </nav>

      <div className="m-6 flex items-center justify-between">
        <h1 className="text-4xl font-bold uppercase">Trang quản lý bài viết</h1>
        <Link href="/admin/posts/create" className="rounded-md bg-gray-300 px-4 py-2 font-bold text-gray-800 transition-colors hover:bg-gray-400">Tạo bài viết mới</Link>
      </div>

      <div className="flex justify-center flex-col">
        <AdminTable columns={col} data={posts} onDetail={handleDetail} onEdit={handleEdit} onDel={handleDel} />
